...
string RetCodeName[13] = {"DDS_RETCODE_OK",
                          "DDS_RETCODE_ERROR",
 ...
 **/
string getErrorName(DDS::ReturnCode_t status) { return RetCodeName[status]; }
...
void checkHandle(void *handle, string info) {
  ...
  if (!handle) {
    cerr << "Error in " << info.c_str() << ": Creation failed: invalid handle"
         << endl;
    exit(1);
  }
}
