...
void DDSEntityManager::createWriter() {
  writer = publisher->create_datawriter(
      topic.in(), DATAWRITER_QOS_USE_TOPIC_QOS, NULL, STATUS_MASK_NONE);
...
void DDSEntityManager::createWriter(bool autodispose_unregistered_instances) {
  ...
  writer =
      publisher->create_datawriter(topic.in(), dw_qos, NULL, STATUS_MASK_NONE);
  checkHandle(writer, "DDS::Publisher::create_datawriter");
}
...
void DDSEntityManager::createReader() {
  reader = subscriber->create_datareader(
      topic.in(), DATAREADER_QOS_USE_TOPIC_QOS, NULL, STATUS_MASK_NONE);
...
Topic_ptr DDSEntityManager::getTopic() { return Topic::_duplicate(topic.in()); }
