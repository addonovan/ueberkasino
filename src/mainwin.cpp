...
uc::Player player;
net::Player* partial_response = nullptr;
...
/**
 * Purpose: Presents a window that displays a text box with blackjack
 * instructions
 */
void howButton_callback( Fl_Widget *, void *a ) {
    ...
    tbuff->text( 
        ...
        "Dealer deals 2 cards to the players face up\nand two to himself ( 1 card "
        "face up, the other face down ).\n\n"

        "Blackjack card values: Numerical cards count their face value in "
        "blackjack.\n"
        ...
        "Double Down: Take only one more card from the dealer and double the "
        "orignal bet.\n"
        "Split: Split your orignal hand into two new hands.\n"
        "Can only split when the orignal hand contains two cards of the same "
        "value.\n\n"

        "The dealer acts last and must hit on 16 or less and stand on 17 through "
        "21.\n"
        "Players win when their hand totals higher than dealer’s hand,\nor they "
        "have 21 or less when the dealer busts\n" );
    ...
    sbuff->text( 
        ...
        "AAAAAAAAAAAAAAAAAAAAAACCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"
        "CCC\n"
        ...
        "AAAAAAAAAAAACCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC"
        "CCCCCCCC\n"
...
/**
 * changes the player's bet by the given change, `delta`.
 */
void change_bet( int delta )
{
    // change the player's bet
    player.bet( player.bet() + delta );

    // update the bet text box
    std::string bet = std::to_string( player.bet() );
    tbbuff->text( bet.c_str() );

    // update the balance text box
    std::string balance = std::to_string( ( int ) player.balance() );
    tmbuff->text( balance.c_str() );
}

/**
 * Sends an action across the network using the current `partial_response`
 * that was already created.
 */
void send_action( net::Action action )
{
    // if there's no response to go off of, then the player just hit
    // the button while there was another strategy active
    if ( partial_response == nullptr )
        return;

    // send the finished response
    partial_response->A = action;
    uc::Network::get().send( *partial_response );

    // delete it and reset it back to null
    delete partial_response;
    partial_response = nullptr;
}

/**
 * Purpose: Generates a window to play a blackjack game.
 * Displays buttons to increase the bet, hit, stand, double down, and clear the
 * bet.
 * Contains radio buttons for the playing styles, the default option is manual.
 * Displays text boxes with wallet and bet amounts, and the players' and
 * dealer's cards
 */
void playButton_callback( Fl_Widget *, void *a ) {
    ...
    ********************************************************************************************************/
    Fl_Text_Display::Style_Table_Entry stable[] = {
        {FL_BOLD, FL_HELVETICA, 18, 0u},  {FL_ITALIC, FL_HELVETICA, 18, 0u},
        {FL_BLACK, FL_HELVETICA, 18, 0u}, {FL_BLACK, FL_HELVETICA, 34, 0u},
    ...
    **************************************************************************************************************/
    ...
    hitButton->callback( []( Fl_Widget*, void* ) {
        send_action( net::Action::hitting );
    } );
    ...
    standButton->callback( []( Fl_Widget*, void* ) {
        send_action( net::Action::standing );
    } );
    ...
    dbldownButton->callback( []( Fl_Widget*, void* ) {
        send_action( net::Action::doubling );
    } );
    ...
    **************************************************************************************************************/
    ...
    Fl_Round_Button *manualButton =
        new Fl_Round_Button( 625, 150, 20, 20, "Manual" );
    ...
    manualButton->callback( []( Fl_Widget*, void* ) {
        player.strategy( new uc::ManualStrategy );
    } );
    ...
    Fl_Round_Button *conservativeButton =
        new Fl_Round_Button( 625, 175, 20, 20, "Conservative" );
    ...
    conservativeButton->callback( []( Fl_Widget*, void* ) {
        player.strategy( new uc::ConservativeStrategy );
    } );

    Fl_Round_Button *recklessButton =
        new Fl_Round_Button( 625, 200, 20, 20, "Reckless" );
    ...
    recklessButton->callback( []( Fl_Widget*, void* ) {
        player.strategy( new uc::RecklessStrategy );
    } );

    Fl_Round_Button *traditionalButton =
        new Fl_Round_Button( 625, 225, 20, 20, "Traditional" );
    ...
    traditionalButton->callback( []( Fl_Widget*, void* ) {
        player.strategy( new uc::BasicStrategy );
    } );
    ...
    **************************************************************************************************************/
    ...
    bet1button->callback( []( Fl_Widget*, void* ){
        change_bet( 1 );
    } );
    ...
    bet5button->callback( []( Fl_Widget*, void* ) {
        change_bet( 5 );
    } );
    ...
    bet10button->callback( []( Fl_Widget*, void* ) {
        change_bet( 10 );
    } );
    ...
    bet50button->callback( []( Fl_Widget*, void* ) {
        change_bet( 50 );
    } );
    ...
    betClear->callback( []( Fl_Widget*, void* ) {
        change_bet( -player.bet() );
    } );
    ...
    ************************************************************************************************************/
    ...
    // when we get a game update, we'll try to update our player object
    uc::Network::get().on_game_update( []( Game game ) { 
        player.from( game );

        // TODO display updates from the game packet here

        net::Player response = player.to();
        
        // The manual mode will always return an idle state, meaning we have
        // to wait for the user to actually press a button
        if ( response.A != net::Action::idle )
        {
            // not manual? Just send back the response immediately
            uc::Network::get().send( response );
        }
        else
        {
            partial_response = new net::Player;
            memcpy( partial_response, &response, sizeof( response ) );
        }
    } );

    // listen for dealers on our network and print out their information
    // when we find one
    uc::Network::get().on_dealer_update( []( Dealer dealer ) {
        std::string uid{ dealer.uid, net::UUID_LENGTH };
        std::string name{ dealer.name, 32 };

        std::cout << "Found a dealer!" << std::endl;
        std::cout << "uuid = " << uid << std::endl;
